...
from drf_yasg import openapi                                                # API 제목, 버전, 연락처 정보 등과 같은 메타데이터를 정의

schema_view = get_schema_view(                                              # API 문서를 위한 스키마 뷰를 생성
    openapi.Info(                                                           # API에 대한 메타데이터를 정의
...
urlpatterns = [
    #Swagger UI를 API 문서를 위해 렌더링하도록 지정
    #cache_timeout=0은 Swagger UI가 캐시되지 않도록 함
